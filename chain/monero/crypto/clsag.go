...
func CLSAGSign(ctx *CLSAGContext) (*CLSAGSignature, error) {
	...
	if ctx.Rand == nil {
		// No true-RNG fallback: nonces must come from a caller-supplied
		// deterministic reader so signing is reproducible.
		return nil, fmt.Errorf("CLSAG signing requires a deterministic random reader")
	}
	l := ctx.SecretIndex
	p := ctx.SecretKey
	z := ctx.ZKey
