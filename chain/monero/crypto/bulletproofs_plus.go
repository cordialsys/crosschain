...
func BulletproofPlusProve(amounts []uint64, masks [][]byte, randReader ...io.Reader) (*BulletproofPlus, []*edwards25519.Point, error) {
	// A deterministic reader is required: falling back to a true RNG would
	// silently produce a different proof on every build.
	var rng io.Reader
	if len(randReader) > 0 && randReader[0] != nil {
		rng = randReader[0]
	} else {
		return nil, nil, fmt.Errorf("BP+ prove requires a deterministic random reader")
	}
	m := len(amounts)
	if m == 0 || m > maxM {
...
func randomScalarFrom(rng io.Reader) *edwards25519.Scalar {
	...
	if rng == nil {
		// No true-RNG fallback: every caller must supply a deterministic
		// reader (validated at the exported entry points).
		panic("randomScalarFrom requires a random reader")
	}
	// A short read would silently weaken the scalar entropy; the
	// deterministic keccak RNG never fails.
	if _, err := io.ReadFull(rng, entropy); err != nil {
		panic(fmt.Sprintf("scalar rng failed: %v", err))
	}
	wide := make([]byte, 64)
