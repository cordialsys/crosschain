...
func TestBulletproofsPlusProveAndVerify(t *testing.T) {
	...
	proof, err := BPPlusProvePureGo([]uint64{amount}, [][]byte{mask}, newDetRNG(Keccak256([]byte("bp-test-1"))))
	require.NoError(t, err)
	...
	proof2, err := BPPlusProvePureGo([]uint64{500000000, 500000000}, [][]byte{mask, mask2}, newDetRNG(Keccak256([]byte("bp-test-2"))))
	require.NoError(t, err)
