...
func (s *CrosschainTestSuite) TestFetchTxInfo() {
	...
	for _, v := range vectors {
		...
		txInfo, err := client.FetchTxInfo(s.Ctx, xc.TxHash(v.val.TxID))
...
func (s *CrosschainTestSuite) TestInvalidTxFetchTxInfo() {
	...
	_, err := client.FetchTxInfo(s.Ctx, xc.TxHash("invalidhash"))
...
func (s *CrosschainTestSuite) TestTransfers() {
	require := s.Require()
	...
	for _, v := range vectors {
		...
		require.NoError(err)
		input, err := client.FetchTxInput(context.Background(), xc.Address(from), xc.Address(to), xc.AmountBlockchain{})
		require.NoError(err)
