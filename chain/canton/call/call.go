...
func NewCall(cfg *xc.ChainBaseConfig, method xccall.Method, msg json.RawMessage, signingAddress xc.Address) (*TxCall, error) {
	...
	contractID, err := parseContractID(method, msg)
	if err != nil {
		return nil, err
	}
...
func parseContractID(method xccall.Method, msg json.RawMessage) (string, error) {
	switch method {
	case xccall.OfferAccept:
		var payload xccall.OfferAcceptCall
	...
	case xccall.SettlementComplete:
		var payload xccall.SettlementCompleteCall
		if err := json.Unmarshal(msg, &payload); err != nil {
			return "", fmt.Errorf("could not parse settlement complete call: %w", err)
		}
		if payload.ContractID == "" {
			return "", fmt.Errorf("missing contract_id")
		}
		return payload.ContractID, nil
	default:
		return "", fmt.Errorf("unsupported Canton call method %q", method)
	}
}
