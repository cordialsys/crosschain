...
const (
	...
	tokenMemoMetadataKey = "splice.lfdecentralizedtrust.org/reason"
)
...
func transferMemo(args xcbuilder.TransferArgs) string {
	memo, ok := args.GetMemo()
	if !ok {
		return ""
	}
	return memo
...
func lockedHoldingExpiresAt(created *v2.CreatedEvent) (time.Time, bool) {
	if created == nil {
		return time.Time{}, false
	}
	ts, ok := recordTimestampRecursive(created.GetCreateArguments(), "expiresAt", 4)
	if !ok {
		return time.Time{}, false
	}
	return ts.Time, true
...
func recordTimestampRecursive(record *v2.Record, field string, depth int) (recordTime, bool) {
	if record == nil || depth <= 0 {
		return recordTime{}, false
	}
	if ts, ok := recordTimestamp(record, field); ok {
		return ts, true
	}
	for _, recordField := range record.GetFields() {
		if ts, ok := recordValueTimestampRecursive(recordField.GetValue(), field, depth-1); ok {
			return ts, true
		}
	}
	return recordTime{}, false
...
func recordValueTimestampRecursive(value *v2.Value, field string, depth int) (recordTime, bool) {
	if value == nil || depth <= 0 {
		return recordTime{}, false
	}
	if record := value.GetRecord(); record != nil {
		return recordTimestampRecursive(record, field, depth)
	}
	if variant := value.GetVariant(); variant != nil {
		return recordValueTimestampRecursive(variant.GetValue(), field, depth-1)
	}
	if optional := value.GetOptional(); optional != nil {
		return recordValueTimestampRecursive(optional.GetValue(), field, depth-1)
	}
	if list := value.GetList(); list != nil {
		for _, element := range list.GetElements() {
			if ts, ok := recordValueTimestampRecursive(element, field, depth-1); ok {
				return ts, true
			}
		}
	}
...
func optionalTextValue(value string) *v2.Value {
	optional := &v2.Optional{}
	if value != "" {
		optional.Value = cantonproto.TextValue(value)
	}
	return &v2.Value{Sum: &v2.Value_Optional{Optional: optional}}
}
			...
			Create: &v2.CreateCommand{
				...
				CreateArguments: &v2.Record{
					Fields: []*v2.RecordField{
						...
						cantonproto.Field("description", cantonproto.TextValue(transferMemo(args))),
		...
		Command: &v2.Command_Exercise{
			Exercise: &v2.ExerciseCommand{
				...
				ChoiceArgument: cantonproto.RecordValue(
					...
					cantonproto.Field("description", optionalTextValue(transferMemo(args))),
				),
			},
		},
	}
...
func BuildTokenStandardTransferCommand(
	...
	InstrumentAdmin, InstrumentID, ok := cantonclientconfig.TokenRegistryKey(contract).Parts()
	if !ok {
		return nil, fmt.Errorf("invalid Canton token contract %q, expected <instrument-admin>#<instrument-id>", contract)
	}
	return BuildTokenStandardTransferCommandForInstrument(
		args,
		transferPackageID,
		factoryContractID,
		choiceContextData,
...
func BuildTokenStandardTransferCommandForInstrument(
	args xcbuilder.TransferArgs,
	transferPackageID string,
	factoryContractID string,
	choiceContextData map[string]any,
				...
				ChoiceArgument: cantonproto.RecordValue(
					...
					cantonproto.Field("transfer", cantonproto.RecordValue(
						...
						cantonproto.Field("meta", cantonproto.RecordValue(
							cantonproto.Field("values", tokenMetadataTextMapValue(args)),
						)),
					)),
					cantonproto.Field("extraArgs", cantonproto.RecordValue(
						cantonproto.Field("context", choiceContextValue),
						cantonproto.Field("meta", cantonproto.RecordValue(
							cantonproto.Field("values", emptyTokenMetadataTextMapValue()),
						)),
					)),
				),
			},
...
func tokenMetadataValues(args xcbuilder.TransferArgs) map[string]string {
	memo := transferMemo(args)
	if memo == "" {
		return map[string]string{}
	}
	return map[string]string{tokenMemoMetadataKey: memo}
}

func tokenMetadataTextMapValue(args xcbuilder.TransferArgs) *v2.Value {
	entries := []*v2.TextMap_Entry{}
	memo := transferMemo(args)
	if memo != "" {
		entries = append(entries, &v2.TextMap_Entry{
			Key:   tokenMemoMetadataKey,
			Value: cantonproto.TextValue(memo),
		})
	}
	return &v2.Value{
		Sum: &v2.Value_TextMap{
...
func emptyTokenMetadataTextMapValue() *v2.Value {
	return &v2.Value{
		Sum: &v2.Value_TextMap{
			TextMap: &v2.TextMap{Entries: []*v2.TextMap_Entry{}},
		},
...
func tokenAnyTextValue(text string) *v2.Value {
	return &v2.Value{
...
func tokenAnyValueToProto(value any) (*v2.Value, error) {
	...
	case "AV_Text":
		...
		return tokenAnyTextValue(text), nil
	case "AV_Int":
		return &v2.Value{
			Sum: &v2.Value_Variant{Variant: &v2.Variant{
				Constructor: tag,
