...
func TestGetAddressFromPublicKey(t *testing.T) {
	...
	name, fingerprint, err := address.ParsePartyID(addr)
	require.NoError(t, err)
...
func TestFingerprintFromPublicKey(t *testing.T) {
	...
	fingerprint, err := address.FingerprintFromPublicKey(publicKeyBytes)
	require.NoError(t, err)
...
func TestFingerprintFromPartyID(t *testing.T) {
	...
	fingerprint, err := address.FingerprintFromPartyID(addr)
	require.NoError(t, err)
	require.Equal(t, "122079aa518eac66dcd662887155c5c7ee36d3b62e38ed0ded2ddc0c7050460bccc8", fingerprint)
}
...
func TestParsePartyID(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			name, fp, err := address.ParsePartyID(tt.addr)
			if tt.expectErr {
				require.Error(t, err)
...
func TestFingerprintLength(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name:        "all-zero bytes",
			seed:        0,
			expectedFP:  "1220ea618da83b6c6b2c4557ffa17d722045169f52b8f50f3b31fc867e266de7e53d",
			expectedLen: 68,
		},
		{
			name:        "all-one bytes",
			seed:        1,
			expectedFP:  "1220974cb80e78f2fea077628a02faa4c57d68a65036eea27fb3463088a1c8527a99",
			expectedLen: 68,
		},
		{
			name:        "all-0x7f bytes",
			seed:        127,
			expectedFP:  "122087fab42073577d1d066f0cc217b347aedf5a73ee5feaa75d5e96538dad977b91",
			expectedLen: 68,
		},
		{
			name:        "all-0xff bytes",
			seed:        255,
			expectedFP:  "122044e19d94c296e8397d61e759ff1692e5dff8efbcd70d7a9b4033d8b4a259ccd0",
			expectedLen: 68,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, fp, err := address.ParsePartyID(addr)
			require.NoError(t, err)
			require.Equal(t, tt.expectedLen, len(fp), "fingerprint must always be 68 chars")
			require.Equal(t, tt.expectedFP, fp)
		})
	}
}
