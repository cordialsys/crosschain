...
func (builder TxBuilder) NewNativeTransfer(from xc.Address, to xc.Address, amount xc.AmountBlockchain, input xc.TxInput) (xc.Tx, error) {
	...
	sourceAccount, err := xdr.NewMuxedAccount(xdr.CryptoKeyTypeKeyTypeEd25519, txInput.PublicKey)
	if err != nil {
		return &xlmtx.Tx{}, fmt.Errorf("invalid public key: %w", err)
	}

	// Native transfer is a single operation
	fee := max(txInput.MaxFee, txInput.BaseFee)

	preconditions := xlmtx.Preconditions{
		TimeBounds: xlmtx.NewTimeout(30),
	}
	...
	if err != nil {
		return &xlmtx.Tx{}, fmt.Errorf("failed to set muxed account addres: %w", err)
	}
	...
	if err != nil {
		return &xlmtx.Tx{}, errors.New("failed to create transaction envelope")
	}
