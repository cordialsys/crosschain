...
type HandshakeMessage struct {
	...
	Key                      string      `json:"key"`     // can be empty, producer key, or peer key
	Time                     Tstamp      `json:"time"`    // time?!
	Token                    Checksum256 `json:"token"`   // digest of time to prove we own the private `key`
	Signature                string      `json:"sig"`     // can be empty if no key, signature of the digest above
...
type BlockState struct {
	...
	AdditionalSignatures      []string                      `json:"additional_signatures"`

	SignedBlock *SignedBlock `json:"block,omitempty" eos:"optional"`
	Validated   bool         `json:"validated"`

	// EOSIO 1.x
	BlockSigningKeyV1 *string `json:"block_signing_key,omitempty" eos:"-"`
}
