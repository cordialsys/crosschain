...
type TxInput struct {
	...
	// For TAO alpha token transfers - positions of alpha held across hotkeys on a subnet
	AlphaPositions []AlphaPosition `json:"alpha_positions,omitempty"`
...
// AlphaPosition represents a single alpha token holding on a specific hotkey
type AlphaPosition struct {
