...
func (client *Client) FetchTransferInput(ctx context.Context, args xcbuilder.TransferArgs) (xc.TxInput, error) {
	...
	feeRates, err := client.client.GetFeeEstimate()
	if err != nil {
		return nil, err
	}

	txBuilder, err := builder.NewTxBuilder(client.chainCfg.Base())
	if err != nil {
		return nil, err
	}
	...
	if client.chainCfg.ChainGasMultiplier > 0.01 {
		txInput.Mass = uint64(float64(transactionMass) * client.chainCfg.ChainGasMultiplier)
	}
	txInput.FeePerGram = feeRates.GetMostNormalFeeEstimate()
	txInput.MinFee = client.chainCfg.GasBudgetMinimum.ToBlockchain(int32(client.chainCfg.Decimals))
...
func (client *Client) SubmitTx(ctx context.Context, txInput xc.Tx) error {
	...
	response, err := client.client.SubmitTransaction(serializedSigned)
	if err != nil {
		return err
	}
...
func (client *Client) FetchDecimals(ctx context.Context, contract xc.ContractAddress) (int, error) {
	...
	if contract != "" && contract != xc.ContractAddress(client.chain) {
...
func (client *Client) FetchBlock(ctx context.Context, args *xclient.BlockArgs) (*xclient.BlockWithTransactions, error) {
	...
	if !ok {
		latestBlueScore, err := client.client.GetVirtualChainBlueScore()
		if err != nil {
			return nil, err
		}
	...
	blocks, err := client.client.GetBlocksFromBlockScore(height)
	if err != nil {
		return nil, err
	}
	firstBlock := blocks[0]
