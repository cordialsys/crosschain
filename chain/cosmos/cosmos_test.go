...
func (s *CrosschainTestSuite) TestIsEVMOS() {
	require := s.Require()
	is := isEVMOS(&xc.ChainConfig{Chain: "ETH", Driver: xc.DriverEVM})
	require.False(is)

	is = isEVMOS(&xc.ChainConfig{Chain: "ATOM", Driver: xc.DriverCosmos})
	require.False(is)

	is = isEVMOS(&xc.ChainConfig{Chain: "LUNA", Driver: xc.DriverCosmos})
	require.False(is)

	is = isEVMOS(&xc.ChainConfig{Chain: "XPLA", Driver: xc.DriverCosmos})
	require.False(is)

	is = isEVMOS(&xc.ChainConfig{Chain: "XPLA", Driver: xc.DriverCosmosEvmos})
	require.True(is)
}
...
func (s *CrosschainTestSuite) TestGetPublicKey() {
	...
	pubKey := getPublicKey(&xc.ChainConfig{Driver: xc.DriverCosmos}, []byte{})
	require.Exactly(&secp256k1.PubKey{Key: []byte{}}, pubKey)

	pubKey = getPublicKey(&xc.ChainConfig{Driver: xc.DriverCosmosEvmos}, []byte{})
...
func (s *CrosschainTestSuite) TestGetSighash() {
	...
	sighash := getSighash(&xc.ChainConfig{Driver: xc.DriverCosmos}, []byte{})
	// echo -n '' | openssl dgst -sha256
	require.Exactly("e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855", hex.EncodeToString(sighash))

	sighash = getSighash(&xc.ChainConfig{Driver: xc.DriverCosmosEvmos}, []byte{})
