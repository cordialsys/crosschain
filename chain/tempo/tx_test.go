...
func feeTokenFixture(t *testing.T) (*xc.ChainConfig, xcbuilder.TransferArgs, *TxInput) {
	t.Helper()
	chain := xc.NewChainConfig("TEMPO").WithChainID("42431")
...
func TestFeeTokenSelectionIsSigned(t *testing.T) {
	chain, args, input := feeTokenFixture(t)
	first, err := buildTempoTransfer(chain, args, input)
	require.NoError(t, err)
	...
	second, err := buildTempoTransfer(chain, otherArgs, input)
	require.NoError(t, err)
...
func TestFeeContractAndFeePayerMatchesOx(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, json.Unmarshal(fixture, &vector))
	unsignedEnvelope := tx.(*Tx).envelope
	unsignedEnvelope.FeeToken, unsignedEnvelope.FeePayer = nil, []byte{0}
	unsigned, err := unsignedEnvelope.encode(tempoTransactionType)
	require.NoError(t, err)
	...
	wrongSender, err := crypto.Sign(requests[0].Payload, payerKey)
	require.NoError(t, err)
	require.ErrorContains(t, tx.SetSignatures(&xc.SignatureResponse{Signature: wrongSender}), "expected signer")
	require.NoError(t, tx.SetSignatures(senderResponse))
	_, err = tx.Serialize()
	require.ErrorContains(t, err, "requires sender and fee-payer signatures")
...
func TestFeeTokenSignatureValidation(t *testing.T) {
	chain, args, input := feeTokenFixture(t)
	tx, err := buildTempoTransfer(chain, args, input)
	require.NoError(t, err)
...
func buildTempoTransfer(chain *xc.ChainConfig, args xcbuilder.TransferArgs, input *TxInput) (xc.Tx, error) {
	b, err := NewTxBuilder(chain.Base())
	if err != nil {
		return nil, err
	}
	return b.Transfer(args, input)
}
