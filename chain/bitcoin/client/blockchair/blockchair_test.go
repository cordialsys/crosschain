...
func (s *ClientTestSuite) TestNewClient() {
	require := s.Require()
	asset := &xc.ChainConfig{Chain: xc.BTC, Net: "testnet", AuthSecret: "1234", Provider: string(bitcoin.Blockchair)}
	client, err := bitcoin.NewClient(asset)
	require.NotNil(client)
	require.NoError(err)

	asset = &xc.ChainConfig{Chain: xc.BTC, Net: "testnet", AuthSecret: "", Provider: string(bitcoin.Blockchair)}
