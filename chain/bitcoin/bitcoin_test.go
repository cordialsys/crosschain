...
func (s *CrosschainTestSuite) TestFetchTxInput() {
	...
	for _, v := range testcases {
		...
		defer close()
		asset := &xc.AssetConfig{NativeAsset: xc.BTC, URL: server.URL, Net: "testnet", AuthSecret: "1234"}
...
func (s *CrosschainTestSuite) TestNewClient() {
	require := s.Require()
	asset := &xc.AssetConfig{NativeAsset: xc.BTC, Net: "testnet", AuthSecret: "1234"}
	client, err := NewClient(asset)
	require.NotNil(client)
	require.NoError(err)

	asset = &xc.AssetConfig{NativeAsset: xc.BTC, Net: "testnet", AuthSecret: ""}
