...
func (s *CrosschainTestSuite) TestNewAddressBuilder() {
	...
	for _, nativeAsset := range UXTO_ASSETS {
		builder, err := NewAddressBuilder(&xc.NativeAssetConfig{Asset: string(nativeAsset)})
...
func (s *CrosschainTestSuite) TestNewTxBuilder() {
	require := s.Require()
	for _, nativeAsset := range UXTO_ASSETS {
		builder, err := NewTxBuilder(&xc.NativeAssetConfig{Asset: string(nativeAsset)})
		require.NotNil(builder)
		require.NoError(err)
	}
}
...
func (s *CrosschainTestSuite) TestFetchTxInput() {
	...
	for _, v := range testcases {
		...
		defer close()
		asset := &xc.NativeAssetConfig{Asset: string(xc.BTC), URL: server.URL, Net: "testnet", AuthSecret: "1234"}
...
func (s *CrosschainTestSuite) TestNewNativeTransfer() {
	...
	for _, addr := range []string{
		...
		for _, native_asset := range []xc.NativeAsset{
			...
			asset := &xc.NativeAssetConfig{Asset: string(native_asset), Net: "testnet"}
...
func (s *CrosschainTestSuite) TestNewTokenTransfer() {
	require := s.Require()
	asset := &xc.NativeAssetConfig{Asset: string(xc.BTC), Net: "testnet"}
	builder, _ := NewTxBuilder(asset)
	from := xc.Address("mpjwFvP88ZwAt3wEHY6irKkGhxcsv22BP6")
	to := xc.Address("tb1qtpqqpgadjr2q3f4wrgd6ndclqtfg7cz5evtvs0")
	amount := xc.NewAmountBlockchainFromUint64(1)
...
func (s *CrosschainTestSuite) TestNewTransfer_Token() {
	require := s.Require()
	asset := &xc.TokenAssetConfig{Asset: string(xc.BTC), NativeAssetConfig: &xc.NativeAssetConfig{Net: "testnet"}}
	builder, _ := NewTxBuilder(asset)
	from := xc.Address("mpjwFvP88ZwAt3wEHY6irKkGhxcsv22BP6")
	to := xc.Address("tb1qtpqqpgadjr2q3f4wrgd6ndclqtfg7cz5evtvs0")
...
func (s *CrosschainTestSuite) TestNewClient() {
	require := s.Require()
	asset := &xc.NativeAssetConfig{Asset: string(xc.BTC), Net: "testnet", AuthSecret: "1234"}
	client, err := NewClient(asset)
	require.NotNil(client)
	require.NoError(err)

	asset = &xc.NativeAssetConfig{Asset: string(xc.BTC), Net: "testnet", AuthSecret: ""}
	_, err = NewClient(asset)
