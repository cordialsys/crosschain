...
func (s *CrosschainTestSuite) TestFetchTxInput() {
	...
	for _, v := range testcases {
		...
		defer close()
		asset := &xc.ChainConfig{Chain: xc.BTC, URL: server.URL, Net: "testnet", AuthSecret: "1234", Provider: string(Blockchair)}
...
func (s *CrosschainTestSuite) TestNewClient() {
	require := s.Require()
	asset := &xc.ChainConfig{Chain: xc.BTC, Net: "testnet", AuthSecret: "1234", Provider: string(Blockchair)}
	client, err := NewClient(asset)
	require.NotNil(client)
	require.NoError(err)

	asset = &xc.ChainConfig{Chain: xc.BTC, Net: "testnet", AuthSecret: "", Provider: string(Blockchair)}
